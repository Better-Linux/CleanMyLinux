...
export default function DashboardView() {
  ...
  const updatesLoading = useAppManagerStore((state) => state.updatesLoading);
  ...
  const showUpdates = updatesLoading || updateCount > 0;
  ...
  return (
    <div
      className={`w-full max-w-6xl flex flex-col not-xl:px-10 h-full ${!hasAlerts ? "justify-center" : "justify-evenly"}`}
    >
      {/* Main Header */}
      <div
        className={`flex flex-col items-center justify-center relative transition-all duration-500 ${!hasAlerts ? "mb-12" : "mb-6"}`}
      >
        <h1
      ...
      {hasAlerts && (
        <div className="w-full grid grid-cols-1 md:grid-cols-12 gap-6 mt-8">
          ...
          {showUpdates && (
            <div
              className={`h-full ${showBoth ? "md:col-span-6" : "md:col-span-6 md:col-start-4"}`}
            >
                ...
                {updatesLoading ? (
                  <div className="flex-1 flex flex-col h-full">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h2 className="text-[20px] font-semibold text-white mb-1.5 leading-tight drop-shadow-md flex items-center gap-2">
                          Checking for Updates
                        </h2>
                        <p className="text-[#a1b0cb] text-[13px] leading-relaxed font-medium">
                          Looking for security patches and package releases...
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 space-y-3 mt-4">
                      {Array.from({ length: 4 }).map((_, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-3 px-3 py-2 rounded-2xl bg-white/5 animate-pulse"
                        >
                          <div className="w-9 h-9 rounded-xl bg-white/10 shrink-0"></div>
                          <div className="h-3 bg-white/10 rounded-full w-28"></div>
                          <div className="h-4 bg-white/10 rounded-full w-12 ml-auto"></div>
                        </div>
                      ))}
                    </div>

                    <div className="flex justify-end pt-4 mt-auto">
                      <div className="h-9 w-28 bg-white/10 rounded-xl animate-pulse"></div>
                    </div>
                  </div>
                ) : (
                  <>
                    <h2 className="text-[20px] font-semibold text-white mb-1 leading-tight drop-shadow-md">
                      {updateCount} Application Update
                      {updateCount !== 1 ? "s" : ""}
                      <br />
                      Available
                    </h2>
                    ...
                    >
                      <div className="space-y-2.5">
                        {updates.slice(0, 4).map((update, i) => {
                          ...
                          const grad =
                            fallbackColors[
                              update.name.charCodeAt(0) % fallbackColors.length
                            ];
                          return (
                            <div
                              key={i}
                              className="flex items-center gap-3 px-3 py-2 rounded-2xl bg-white/5 hover:bg-white/8 transition-colors"
                            >
                              <div className="relative shrink-0 w-9 h-9 rounded-xl overflow-hidden">
                                {iconUrl ? (
                                  ...
                                  <div
                                    className={`w-full h-full bg-linear-to-br ${grad} flex items-center justify-center`}
                                  >
              ...
              <div className="glass-panel rounded-3xl p-8 flex flex-col justify-between relative overflow-hidden group hover:shadow-[0_15px_40px_rgba(0,0,0,0.5)] transition-all duration-300 h-full min-h-55">
                <div>
                  <div className="flex justify-between items-start mb-4 gap-4">
                    <h2 className="text-[20px] font-semibold text-white drop-shadow-md leading-tight">
                      {unusedApps.length} Unused Application
                      {unusedApps.length !== 1 ? "s" : ""}
                      <br />
                      Detected
                    </h2>
                    <div className="flex items-center -space-x-4 pr-2 shrink-0">
                      {unusedApps
                        ...
                        .map((app, i) => (
                          <div
                            key={i}
                            className="w-11 h-11 rounded-xl flex items-center justify-center transform transition-all duration-300 group-hover:-translate-y-1 relative overflow-hidden"
                          ...
                          >
                            {app.icon ? (
                              ...
                              <span className="font-bold text-[15px] text-white/40">
                  ...
                  <p className="text-[#a1b0cb] text-[13px] font-medium leading-relaxed max-w-[85%]">
                    These applications haven't been used recently. Removing them
                    could reclaim up to {formatSizeNice(unusedSizeBytes)} of
                    disk space.
                  </p>
                </div>
                ...
                <div className="flex justify-end pt-4 mt-auto">
                  <button
                    ...
                    className="glass-btn px-8 py-2.5 rounded-xl text-[13px] font-semibold tracking-wide mt-10"
                  >
                    Review Unused Apps
                  </button>
                </div>
